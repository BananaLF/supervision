...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"Org3MSP":.[1]}}}}}' config.json ./organizations/commiterOrganizations/org3.example.com/org3.json > modified_config.json
{ set +x; } 2>/dev/null
...
signConfigtxAsCommiterOrg 1 org3_update_in_envelope.pb
