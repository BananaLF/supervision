...
function printHelp() {
  ...
  if [ "$USAGE" == "ca" ]; then
    ...
    println "    -s <dbtype> - Commiter state database to deploy: goleveldb (default) or couchdb"
    ...
    println "    -i <imagetag> - Docker image tag of mhchain to deploy (defaults to \"latest\")"
    println "    -cai <ca_imagetag> - Docker image tag of mhchain CA to deploy (defaults to \"${CA_IMAGETAG}\")"
  ...
  elif [ "$USAGE" == "consensus" ]; then
    println "Usage: "
    println "  network.sh \033[0;32mconsensus\033[0m [Flags]"
    println
    println "    Flags:"
    println "    -c <channel name> - Name of channel to create (defaults to \"mychannel\")"
    println "    -r <max retry> - CLI times out after certain number of attempts (defaults to 5)"
    ...
    println "    Modes:"
    println "      \033[0;32mup\033[0m - Bring up mhchain consensus and commiter nodes. No channel is created"
    println "      \033[0;32mup createChannel\033[0m - Bring up mhchain network with one channel"
